...
export default function AdminInvoices() {
  ...
  const [form, setForm] = useState({
    ...
    status: "DRAFT", notes: "", items: [{ product_id: "", quantity: 1 }],
  });
  ...
  const toTemplateInvoice = (detail) => {
    ...
    return {
      ...
      client: { pic_name: cl.pic_name, company_name: cl.company_name, email: cl.email, address: cl.address },
    };
  };
  ...
  const fetchInvoiceDetail = async (id) => {
    const res = await getInvoiceById(id, accessToken);
        ...
        <div className="relative mb-6 overflow-hidden rounded-2xl bg-gradient-to-r from-[#3525cd] to-[#5b44f3] p-6 text-white shadow-lg">
          ...
          <div className="relative flex flex-wrap items-center justify-between gap-4">
            <div>
              ...
              <p className="mt-1 text-sm text-white/80">Kelola invoice client dan status pembayaran.</p>
            </div>
            <button
              type="button"
              onClick={openAdd}
          ...
          <div className="flex flex-col gap-4 lg:flex-row lg:items-center">
            <div className="relative flex-1">
              ...
              <input
                ...
                placeholder="Cari nomor invoice, client, atau produk..."
                      ...
                      <tr key={inv.id} className="border-b border-[#e0e3e5] transition last:border-b-0 hover:bg-[#faf9fc]">
                        ...
                        <td className="px-4 py-3">
                          <div className="flex items-center justify-end gap-1">
                            <button type="button" onClick={async () => { try { const detail = await fetchInvoiceDetail(inv.id); setDetailTarget(detail); } catch (err) { setError(err.message); } }} title="Detail" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-[#3525cd]/10 text-[#3525cd] transition hover:bg-[#3525cd]/20">
                            ...
                            {isSuperadmin ? (
                              <button type="button" onClick={() => setDeleteTarget(inv)} title="Hapus" className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-[#dc2626]/10 text-[#dc2626] transition hover:bg-[#dc2626]/20">
                                <Trash2 className="h-4 w-4" />
                              </button>
                            ) : (
                              <button type="button" onClick={() => setCancelTarget(inv)} title="Batalkan" disabled={inv.status === "CANCELLED" || inv.status === "PAID"} className="flex h-8 w-8 shrink-0 items-center justify-center rounded-md bg-[#dc2626]/10 text-[#dc2626] transition hover:bg-[#dc2626]/20 disabled:cursor-not-allowed disabled:opacity-40">
